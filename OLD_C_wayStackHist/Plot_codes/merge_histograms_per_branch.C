...
void merge_histograms_per_branch() {
    ...
    // Get histogram names from the first file
    ...
    // Process per histogram
    for (const auto& hist_name : hist_names) {
        std::cout << "Processing branch: " << hist_name << std::endl;

        // Create a ROOT file for this histogram branch
        TFile* outFile = new TFile((hist_name + ".root").c_str(), "RECREATE");
        ...
        for (const auto& group : groups) {
            ...
            // Save this group's histogram into the current output file
            if (mergedHist) {
                outFile->cd();
                mergedHist->Write();
                delete mergedHist;
    ...
    std::cout << "All histograms written into separate ROOT files by branch." << std::endl;
}
